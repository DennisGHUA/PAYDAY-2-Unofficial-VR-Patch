			...
			elseif self._belt_state == "reload" and self._hsm:default_state_name() ~= "weapon" then
				...
				if player:movement():current_state():can_grab_mag() then
					...
					if mag_unit then
						...
						--Support for "secondary reload part types" (aka ECP fix)
						if tweak_data.vr.reload_timelines[weap_base.name_id] and tweak_data.vr.reload_timelines[weap_base.name_id].secondary_reload_part_type then
							second_mag = weap_base:spawn_belt_magazine_unit(nil, true)
